...
public class AddonVanilla extends AddonBlank {
    ...
    private boolean breedingCooldown, growupTimer = true;
    ...
    public void updateConfigs(Configuration config) {
        breedingCooldown = config.get("vanilla", "showBreedingCooldown", true, "Show animal breeding cooldown.").setLanguageKey("topaddons.config:vanilla_breeding_cooldown").getBoolean();
        growupTimer = config.get("vanilla", "growupTimer", true, "Show animal grow up timer.").setLanguageKey("topaddons.config:vanilla_growup_timer").getBoolean();
    }
